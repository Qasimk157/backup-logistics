...
// ═══════════════════════════════════════════
// CONFIG — Change this to your FastAPI URL
// ═══════════════════════════════════════════
// const API_BASE_URL = "https://your-fastapi-server.com/api"; // ← Update this
...
const Navigation: React.FC<INavbarCallbacks> = ({
	...
	try {
	  ...
	  if (response.ok) {
		...
		toast.current?.show({
		  ...
		  detail: data.detail || data.message || "Invalid credentials",
		  life: 4000,
		});
	  }
	} catch (error) {
	  toast.current?.show({
		...
		detail: "Unable to reach server. Please try again.",
		life: 4000,
	  });
	} finally {
	...
	try {
	  ...
	  if (response.ok) {
		toast.current?.show({
		  ...
		  detail: data.message || "Welcome to Backup Logistics",
		  life: 4000,
		});
		setShowSignup(false);
		resetSignupForm();
		// Optionally auto-open login
		// setShowLogin(true);
	  } else {
		toast.current?.show({
		  severity: "error",
		  summary: "Registration Failed",
		  detail: data.detail || data.message || "Something went wrong",
		  life: 4000,
		});
	  }
	} catch (error) {
  ...
  // ── Open handlers (close drawer if mobile) ──
  const openLogin = () => {
	toggleDrawer(false);
	setShowLogin(true);
  };
  ...
  const renderMobileDrawer = (
	...
	>
	  ...
	  <List className="bl-nav-drawer-list">
		{navLinks.map((link, index) => (
		  <ListItem
			key={index}
			button
			onClick={() => { link.onClick(); toggleDrawer(false); }}
	  ...
	  <Box className="bl-nav-drawer-actions">
		<button className="bl-nav-btn-login-mobile" onClick={openLogin}>
		  Login
		</button>
		<button className="bl-nav-btn-signup-mobile" onClick={openSignup}>
		  Get Started
		</button>
	  </Box>
	</Drawer>
  ...
  return (
	<>
		...
		<Toolbar className="bl-navbar-toolbar">
		  ...
		  {!isMobile && (
			<Box className="bl-navbar-links">
			  {navLinks.map((link, index) => (
				<button key={index} onClick={link.onClick} className="bl-navbar-link">
				  {link.label}
				</button>
			  ))}
			</Box>
		  ...
		  {!isMobile && (
			<Box className="bl-navbar-actions">
			  <button className="bl-nav-btn-login" onClick={() => setShowLogin(true)}>
				Login
			  </button>
		  ...
		  {isMobile && (
			<IconButton onClick={() => toggleDrawer(true)} className="bl-navbar-hamburger">
			  <MenuIcon />
			</IconButton>
		  )}
		</Toolbar>
	  ...
	  <Dialog
		visible={showLogin}
		onHide={() => { setShowLogin(false); resetLoginForm(); }}
		className="bl-auth-dialog"
		showHeader={false}
		modal
		dismissableMask
		style={{ width: isMobile ? "95vw" : "440px" }}
	  >
		<div className="bl-auth-card">
		  {/* Close button */}
		  <button className="bl-auth-close" onClick={() => { setShowLogin(false); resetLoginForm(); }}>
			<i className="pi pi-times" />
		  </button>

		  {/* Header */}
		  <div className="bl-auth-header">
			<div className="bl-auth-icon">
		  ...
		  <div className="bl-auth-form">
			<div className="bl-auth-field">
			  ...
			  <span className="p-input-icon-left bl-auth-input-wrap">
				...
				<InputText
				  ...
				  className={`bl-auth-input ${loginErrors.email ? "p-invalid" : ""}`}
			...
			<div className="bl-auth-field">
			  ...
			  <Password
				...
				className={`bl-auth-password ${loginErrors.password ? "p-invalid" : ""}`}
				toggleMask
				feedback={false}
				onKeyDown={(e) => e.key === "Enter" && handleLogin()}
			  />
			...
			<div className="bl-auth-options">
			  <div className="bl-auth-remember">
				<Checkbox
				  ...
				  onChange={(e) => setLoginForm({ ...loginForm, remember: e.checked ?? false })}
				  className="bl-auth-checkbox"
				/>
				<label htmlFor="remember" className="bl-auth-remember-label">Remember me</label>
			  </div>
			  <a className="bl-auth-forgot" onClick={() => {
				toast.current?.show({
				  severity: "info",
				  summary: "Reset Password",
				  detail: "Please contact support at shahzaib@backuplogistics.us",
				  life: 5000,
				});
			  }}>
				Forgot password?
		  ...
		  <div className="bl-auth-footer">
			<p>
			  ...
			  <a className="bl-auth-switch" onClick={switchToSignup}>
				Get Started
			  </a>
			</p>
		  </div>
	  ...
	  <Dialog
		visible={showSignup}
		onHide={() => { setShowSignup(false); resetSignupForm(); }}
		className="bl-auth-dialog"
		showHeader={false}
		modal
		dismissableMask
	  ...
	  >
		<div className="bl-auth-card">
		  <button className="bl-auth-close" onClick={() => { setShowSignup(false); resetSignupForm(); }}>
			<i className="pi pi-times" />
		  </button>

		  <div className="bl-auth-header">
			<div className="bl-auth-icon bl-auth-icon-signup">
		  ...
		  <div className="bl-auth-form">
			  ...
			  <div className="bl-auth-field">
				...
				<span className="p-input-icon-left bl-auth-input-wrap">
				  ...
				  <InputText
					...
					className={`bl-auth-input ${signupErrors.fullName ? "p-invalid" : ""}`}
				  />
				</span>
			  ...
			  <div className="bl-auth-field">
				...
				<span className="p-input-icon-left bl-auth-input-wrap">
				  ...
				  <InputText
					...
					placeholder="you@company.com"
					className={`bl-auth-input ${signupErrors.email ? "p-invalid" : ""}`}
				  />
				</span>
			...
			<div className="bl-auth-row">
			  <div className="bl-auth-field">
				...
				<Password
				  ...
				  className={`bl-auth-password ${signupErrors.password ? "p-invalid" : ""}`}
				  toggleMask
				/>
			  ...
			  <div className="bl-auth-field">
				...
				<Password
				  ...
				  className={`bl-auth-password ${signupErrors.confirmPassword ? "p-invalid" : ""}`}
				  toggleMask
				  feedback={false}
				/>
			...
			<div className="bl-auth-terms">
			  <Checkbox
				...
				onChange={(e) => setSignupForm({ ...signupForm, agreeTerms: e.checked ?? false })}
				className="bl-auth-checkbox"
			  />
			  <label htmlFor="agreeTerms" className="bl-auth-terms-label">
				I agree to the{" "}
				<a href="/terms" target="_blank" className="bl-auth-terms-link">Terms & Conditions</a>
				{" "}and{" "}
				<a href="/privacy" target="_blank" className="bl-auth-terms-link">Privacy Policy</a>
			  </label>
			</div>
			...
			<Button
			  ...
			  icon={signupLoading ? "pi pi-spin pi-spinner" : "pi pi-user-plus"}
			  className="bl-auth-submit"
		  ...
		  <div className="bl-auth-footer">
			<p>
			  ...
			  <a className="bl-auth-switch" onClick={switchToLogin}>
				Sign In
			  </a>
			</p>
		  </div>
